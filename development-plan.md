...
- [ ] **Desktop-to-mobile handoff**: on desktop, the try-on modal shows a "Try on mobile" QR code + copy-link button. QR target is the canonical product URL with `?tryon=1&source=qr`; on mobile, that URL auto-opens the try-on modal. Generate QR client-side with an open-source JS library, lazy-loaded only after the modal opens.
- [ ] **Open-source stack only**: no paid Shopify try-on app/plugin. Use open-source/browser libraries: MediaPipe Tasks Vision for face landmarks, Three.js for GLB rendering, and an open-source QR generator. Any third-party code must be vendored/bundled into the app extension or loaded from Shopify-served extension assets, not from paid SaaS widgets.
